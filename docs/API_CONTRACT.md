...
- **Handy-PWA / Mobile – Arbeitsnachweis (Beleg, nicht Monteur-Protokoll):** `dispo/api/mobile/arbeitsnachweis.php` – Auth Token/Basic. Actions: `prefill`, `list`, `get`, `save`, `signature`, `timesheet_preview`, `timesheet_apply`, `pdf_upload`, `pdf`. Envelope `{ ok }`. Fachdaten `snake_case`. Keine QES; SHA-256 nur über Fachinhalt (nicht Signatur-PNG). Laptop-Proxy: `POST /api/laptop_arbeitsnachweis_proxy`. PDF-Engine kanonisch `electron/lib/protocol_pdf.js` (`generateArbeitsnachweisPdfBuffer`).
