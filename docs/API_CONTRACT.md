...
**Abrechnung-Kommentare ändern/löschen (Monteur):** Laptop `POST /api/abrechnung_comment_edit.php` / `POST /api/abrechnung_comment_delete.php` (FormData: `comment_id`, `body` beim Edit, **`job_id`**, optional `bucket`). Dispo: `POST dispo_api/api/abrechnung_comment_edit.php` und `…/abrechnung_comment_delete.php` (JSON `technician_id`, `comment_id`, optional `job_id`, Edit zusätzlich `body`) plus Wrapper `api/monteur_abrechnung_comment_edit.php` / `_delete.php`. Handy: `POST dispo/api/mobile/abrechnung_comment_edit.php` / `_delete.php` (Bearer). Nur eigener Kommentar (`created_by` = Techniker-ID), Auftrag nicht `abgerechnet`. Envelope `{ "ok": true }`. Laptop queued bei Offline `abrechnung_outbox` ops `comment_edit` / `comment_delete`.
...
- **Monteur-Auftrag (dispo_api):** `dispo/dispo_api/api/job.php` – GET liefert unter `job` u. a. **`assigned_to_me`** (bool): der abfragende Monteur ist in `job_technicians` eingetragen (Steuerung von Schreibzugriffen in PWA/Laptop). Hotel-Felder umfassen zusätzlich **`hotel_id`**, **`hotel_comment`**, **`hotel_rating_stars`**, **`hotel_rating_avg`**, **`hotel_rating_count`**. PATCH akzeptiert zusätzlich **`hotel_selection`** (`hotel_id`, optional `comment`, optional `rating_stars` 0..5) und den **Auftragszeitraum** (`start_datetime`, `end_datetime`, optional `date_not_fixed` 0/1). Datum-only (`YYYY-MM-DD`) wird zu Start `00:00:00` / Ende `23:59:59`. Antwort `{ "ok": true, "updated": "schedule", "start_datetime", "end_datetime", "date_not_fixed" }`. Laptop: `PATCH /api/job` schreibt lokal und queued `pending_changes` action `schedule`. Zeitraum-PATCH sendet **`local_job_id`** (SQLite-`jobs.id`) plus **`server_id`** (Dispo-ID); `job_id` allein darf nicht die Kalender-Dispo-ID sein, sonst wird ein anderer lokaler Auftrag getroffen. Gleiche Felder in `dispo/api/mobile/job.php`. **Status `erledigt`:** optionales **`transfer_rel_paths`** (Array relativer Pfade, snake_case). Feld **fehlt** / `null` = Transfer nach PROJEKTE NEU wie bisher (alle stagebaren Dateien). **`[]`** = nichts nach PROJEKTE NEU. Nicht-leere Liste = nur diese Dateien (Match ab `/Montage/<Auftragsordner>/`, Prefix `Dokumente_Monteur/` optional, case-insensitive). Gilt analog für `dispo/api/job.php`, `dispo/api/mobile/job.php`, `dispo/native_app_api/api/job.php`. Laptop schreibt die Auswahl vor dem Statuswechsel als Sidecar `.kukla_transfer_rel_paths.json` im Jobordner und sendet dasselbe Feld bei Sofort-Push und `pending_changes` (inkl. Retry nach Zwischenstufe `in_arbeit`). **Ohne Feld** (volle Kopie): `dispo/api/job_save.php` (Dispo-Web) und `dispo/api/desktop/db_mirror_push.php`.
- **Laptop Abschluss-Auswahl:** `GET /api/dienstreise/finish_file_list?job_id=` → `{ ok, files: [{ rel_path, abs_path, name, folder_label, fn_label, size_bytes }], groups, fn_options }`. `POST /api/dienstreise/finish_extra_files` multipart (`job_id`, optional `fn`, Dateien `file`) speichert unter `Dokumente_Monteur/<FN>/Montage/<Auftragsordner>/Sonstiges/`. `POST /api/dienstreise/finish_and_cleanup` optional `transfer_rel_paths` (gleiche Semantik wie Job-PATCH).
...
- **Handy-PWA / Mobile – Foto in Projektordner:** `dispo/api/mobile/upload_job_photo.php` – `POST` `multipart/form-data` mit `job_id`, optional `fabrikationsnummer`, optional `photo_category` (`Allgemein`/`Angebot`), Datei-Feld `photo` (JPEG/PNG/WebP). Monteur-Token. **Mit FN:** `Dokumente_Monteur/<Anlagenordner>/Montage/<Auftragsordner>/Bilder/`. **Kategorie Allgemein/Angebot:** dieselben FN-Bäume unter `…/Bilder/<Kategorie>/` (ohne FN: Legacy `Dokumente_Monteur/Montage/<Auftrag>/Bilder/<Kategorie>/`). FN-Fotos landen nie in Allgemein/Angebot. Erfolg: JSON `{ "ok": true, "file_name", "relative_path", "job_id", "fabrikationsnummer", "photo_category" }`.
...
- **PROJEKTE-NEU Baum-Export (Laptop `dispo_api`):** `POST dispo_api/api/anlagenstamm_pn_tree_export_chunk.php?technician_id=` JSON `{ page, page_size, since_updated_at? }`. Antwort `{ ok, items: [{ fab, tree, root_name, content_signature, updated_at, truncated, projects_enabled }], total_count, total_pages, delta, server_time }`. Ohne `since_updated_at` voller Katalog; mit Stempel nur FNs mit `tree_fab_meta.updated_at` neuer. `updated_at` wird bei unverändertem Ordner (mtime/Signatur-Skip) **nicht** angehoben.
- **PROJEKTE-NEU Thumb-Export (Laptop `dispo_api`, nicht Handy-PWA):** `POST dispo_api/api/anlagenstamm_pn_thumbs_export_chunk.php?technician_id=` JSON `{ page, page_size` (1–5), `thumb_max` (Default 256), `since_updated_at? }`. Antwort `{ ok, items: [{ fab, thumb_max, content_signature, updated_at, thumbs: [{ rel_path, thumb_max, content_type, thumb_blob_b64, source_size_bytes, source_mtime_unix, updated_at }], keep_rel_paths }], delta, server_time }`. Nur Vorschaubilder, **keine Vollbilder**. Laptop schreibt nach `image_thumb_cache` (`cache_kind=projekte_neu`). PWA bleibt on-demand `thumb=1`.
- **Nur Monteur-Laptop / Dispo `dispo_api` (nicht Handy-PWA):** Zentraler Anlagenstamm lesen/suchen/speichern:
  ...
  - **`POST /api/anlagenstamm_save`** (Fallback) bzw. **IPC `anlagenstamm:save`** → `dispo_api/api/anlagenstamm_monteur_save.php?technician_id=` mit **`baseUrl`**, Auth wie oben, plus **`id`**, **`fabrikationsnummer`**, und Stammfelder (**`type`**, **`leistung`**, **`kraftaufnehmer`**, optional **`kraftaufnehmer_extra`** als JSON-Array von Objekten `{kraftaufnehmer,dms_nr,dms_position,vers_spannung,sensitivitaet}` für Zusatzzeilen, nur Formular/Popup, nicht Listen-Spalte), **`nenngeschwindigkeit`**, **`behaelter_nenninhalt`** (zusätzliche Größe bei Type D-DW / V-DG-1; auf Service-/IBN-Protokollen ersetzt das Feld **v max**), **`material`**, **`tacho`**, **`elektronik`**, **`elektronik_type`**, **`geraete_nummer`**, **`bussystem`**, **`dms_nr`**, **`dms_position`**, **`vers_spannung`**, **`sensitivitaet`**, **`position`**, **`geliefert_ueber`**, **`projekt`**, **`bemerkungen`**. **Partial-Update:** nur Keys, die im JSON-Body vorkommen, werden geschrieben; fehlende Keys behalten den bestehenden DB-Wert (Laptop sendet absichtlich keine leeren Felder via `stripEmpty`). Explizit leerer String löscht das Feld. Web-Save `api/anlagenstamm_save.php` bleibt Full-Update. **`GET api/anlagenstamm_get.php`** und Lookup (`anlagenstamm_lookup_row_by_fab`) liefern dieselbe Feldliste inkl. **`vers_spannung`**, **`sensitivitaet`**, **`elektronik_type`**, **`geraete_nummer`**, **`bussystem`**. **`aktueller_kunde`** / **`letzter_besuch`** nicht setzbar. Erfolg: **`ok`**, **`id`**.
...
- **Handy-PWA / Mobile – Arbeitsnachweis (Beleg, nicht Monteur-Protokoll):** `dispo/api/mobile/arbeitsnachweis.php` – Auth Token/Basic. Actions: `prefill`, `list`, `get`, `draft`, `save`, `signature`, `timesheet_preview`, `timesheet_apply`, `pdf_upload`, `pdf`. Envelope `{ ok }`. Fachdaten `snake_case`. Kopf: `fabrikationsnummern` (JSON-Array `{ fabrikationsnummer, type }`, alle FN des Auftrags, keine Auswahl), `site` = Auftragsadresse (nicht Hotel). Schalter in DB: `naechtigung_beigestellt`, `timesheet_applied`, `language`. `total_km` aus Start/End berechnet, Feld überschreibbar (intern `total_km_manual` wenn abweichend). Arbeitszeile `item_time` als `HH:MM–HH:MM` (UI Von/Bis). Ersatzteil-Kommentar in `description`. Keine QES; SHA-256 nur über Fachinhalt (nicht Signatur-PNG). **Gemeinsamer Entwurf (Handy + Laptop):** ein Beleg je `job_id` + Techniker (`created_by`) in der Dispo-DB. `GET ?action=draft&job_id=` liefert den aktuellen Stand oder `{ ok: true, document: null }`. `save` ohne `id` matched zuerst `local_uuid`, sonst den bestehenden Entwurf desselben Technikers am Auftrag (keine zweite Nummer). Save-Response enthält `local_uuid`. **Laptop:** `GET /api/arbeitsnachweis?job_id=` pullt den Dispo-Entwurf nur, wenn das Remote-Inhaltsgewicht ≥ lokal; sonst bleibt der vollere Laptop-Stand und wird als dirty erneut gequeue't. Queue-Push baut die Save-Payload aus aktueller SQLite (`resolveSavePayload`), nicht aus eingefrorenen Dead-Letter-Snapshots. `POST /api/arbeitsnachweis` schreibt lokal und sofort Dispo (`pending_changes` nur offline). **PWA:** Autosave per `action=save` (debounced); IndexedDB Cache. Remote-Draft wird nicht übernommen, wenn der lokale Draft schwerer ist. `save` überspringt leichtere Payloads (`skipped_empty_overwrite` / `skipped_stale_overwrite`). Proxy bleibt `POST /api/laptop_arbeitsnachweis_proxy` für prefill/signature/timesheet. PDF-Engine kanonisch `electron/lib/protocol_pdf.js` (`generateArbeitsnachweisPdfBuffer`).
...
| Endpunkt | Methode | Kurzbeschreibung |
|----------|---------|------------------|
| `dispo_api/api/serviceprotokoll_defaults.php` | GET | `fabrikationsnummer`, `technician_id` → `{ ok, source: "fn"\|"preset"\|"global"\|"builtin", arbeitsschritte: [{ bezeichnung }], kopf?: { projekt, kopf_pos_nr, kopf_qmax, kopf_type, kopf_dwc }, preset_name?, preset_type_code? }` — Lade-Priorität: FN-Vorlage → Typ-Preset (kompakter Substring in `anlagenstamm.type`, Bindestriche ignoriert, z. B. `DDW` trifft `D-DW-1ASG-800`) → globaler Grundstock → Builtin |
...
| `dispo_api/api/kontrollwiegungsprotokoll_save.php` | POST JSON | `technician_id`, `job_id`, `fabrikationsnummer`, `durchfuehrungsdatum`, `wiegungen[]` → Upsert DB. Pro Wiegung u. a. `in_summe`, `in_pdf` (bool/0/1). **Kopplung:** `in_summe=1` erzwingt `in_pdf=1`. Fehlt `in_pdf` und nicht in Summe → nicht im PDF. Migration 079 speichert beides relational. → `{ ok, protokoll_id }` |
| `dispo_api/api/schleppkettenprotokoll_save.php` | POST JSON | `technician_id`, `job_id`, `fabrikationsnummer`, `durchfuehrungsdatum`, Kopfdaten, `ketten[]` (`in_summe`), `messungen[]` (`in_summe`, `in_pdf`, gleiche Kopplung wie KW) → Upsert DB; Berechnung Prüfkette/Fehler%/Leistung → `{ ok, protokoll_id }` |
...
Masterliste + Typ-Presets (global + privat pro Techniker). Scope analog Textbausteine; keine Kategorien — Gruppierung über Presets (`type_code`, max. 6 Zeichen). Match: kompakter Substring in `anlagenstamm.type` (Kleinbuchstaben, ohne Leerzeichen/`-`/`_`/`.`/`/`). Beispiel: `DDW` trifft `D-DW-1ASG-800`. Bei mehreren Treffern: längster kompakter Code, global vor user.
