...
- **Handy-PWA / Mobile:** `dispo/api/mobile/job.php` – GET `?id=` (Bearer-Token wie `jobs.php`), vollständiger Auftrag inkl. `assigned_to_me`, `technicians_on_job`; Stammdaten wie in der Dispo-Maske u. a. **`customer_email`**, **`billing_*`** (Rechnungsadresse aus `job_billing_addresses`), **`created_at`/`updated_at`**, **`created_by_name`/`updated_by_name`**, Hotel **`hotel_phone`/`hotel_email`/`hotel_website`** aus `job_hotel_addresses` (nicht mehr leer), plus **`hotel_id`**, **`hotel_comment`**, **`hotel_rating_stars`**, **`hotel_rating_avg`**, **`hotel_rating_count`**. Zusätzlich **`rams`** (`null` oder Objekt): **`status`** (`requested` \| `in_progress` \| `submitted` \| `pdf_ready`), **`template_key`**, **`site_country_code`**, **`answers`** (Katalog-IDs je Sektion), **`matrix_severity`** / **`matrix_likelihood`** (1–5 oder `null`), **`can_edit`** (nur zugewiesener Monteur in `requested`/`in_progress`), **`catalog`** (Checkbox-Listen inkl. **`disclaimer_de`** und **`matrix_labels`**). Bei **`can_edit`** optional **`ai_draft`**: **`kind`** **`draft`**, **`source`** (z. B. **`heuristic_v1`** — regelbasiert nach **`job_type`**, keine externe KI), **`answers`** (nur gültige Katalog-IDs als Vorschlag), **`disclaimer_de`** / **`disclaimer_en`**. Entwürfe werden nicht persistiert; Monteur muss auswählen und speichern. Server: **`RAMS_AI_DRAFTS=0`** schaltet **`ai_draft`** ab. Logging: nur **`job_id`**, **`technician_id`**, **`source`** (ohne Freitext aus Auftrag). POST optional Status, Beschreibung, Hotelfelder und **`hotel_selection`** (`hotel_id`, optional `comment`, optional `rating_stars`). Zusätzlich **`rams_save`**: Objekt mit **`answers`**, optional **`matrix_severity`**, **`matrix_likelihood`** (speichert RAMS-Zeile); **`rams_submit`**: `true` schließt die RAMS ab (Matrix beide Werte 1–5 erforderlich). Erfolg u. a. **`updated`:** **`rams`**. Bei Statuswechsel nach `erledigt` erfolgt serverseitig zusätzlich die automatische FN->Hotel-Zuordnung. Auftragsobjekt kann **`montage_abgerechnet`** / **`montage_verrechnet`** (0/1) und **`status`** **`abgerechnet`** enthalten (nach Schema-Migration). Status **`abgerechnet`:** kein Status-Update durch Monteur (`updateJobStatus`); Upload gesperrt in `upload_job_document.php` (HTTP 409, `code`: `job_abgerechnet`).
- **Dispo-Web RAMS anfordern:** `dispo/api/job_rams_request.php` – POST JSON **`job_id`**, **`technician_id`** (muss in `job_technicians` stehen); Session mit **`jobs_save`**. Legt **`job_rams`** an (aus Einsatzadresse: **`site_country_code`**, **`template_key`** DE/EN) oder aktualisiert die Zuweisung nur bei Status **`requested`**.
- **Dispo-Web RAMS-PDF:** `dispo/api/job_rams_generate_pdf.php` – POST JSON **`job_id`**, **`pdf_language`** (`DE` \| `EN`); Session **`jobs_save`**. Nur wenn **`job_rams.status`** **`submitted`** oder bereits **`pdf_ready`** (Neuerzeugung). Schreibt PDF unter **`Dokumente_Dispo/`** im Projektordner, setzt **`pdf_relative_path`**, **`pdf_generated_at`**, **`pdf_generated_by`**, **`status`** → **`pdf_ready`**. Erfolg: **`ok`**, **`pdf_relative_path`**, **`pdf_language`**.
- **Dispo-Web RAMS-PDF Download:** `dispo/api/job_rams_pdf_download.php` – GET **`job_id`**; eingeloggte Dispo mit **`jobs`**-Zugriff. Liefert die Datei aus **`job_rams.pdf_relative_path`** (nur **`pdf_ready`**).
