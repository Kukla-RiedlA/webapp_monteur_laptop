...
- **Nur Monteur-Laptop / Dispo `dispo_api` (nicht Handy-PWA):** Zentraler Anlagenstamm lesen/suchen/speichern:
  - Lokal **`POST /api/anlagenstamm_search`** → `dispo_api/api/anlagenstamm_monteur_search.php?technician_id=` mit JSON **`baseUrl`**, **`serverUsername`**, **`serverPassword`**, **`filter_fn`**, **`filter_type`**, **`filter_aktueller_kunde`**, **`filter_land`** (mindestens eines nicht leer), optional **`page`**, **`page_size`** (max. 100 serverseitig). Erfolg: **`ok`**, **`rows`** (Felder wie `anlagenstamm`), **`total_count`**, **`page`**, **`page_size`**, **`total_pages`**. **`filter_land`:** Subselect auf **`customers.country`** bei Namensgleichheit mit **`anlagenstamm.aktueller_kunde`**.
  - Lokal **`POST /api/anlagenstamm_save`** → `dispo_api/api/anlagenstamm_monteur_save.php?technician_id=` mit JSON **`baseUrl`**, Auth wie oben, plus **`id`**, **`fabrikationsnummer`**, **`type`**, **`leistung`**, **`kraftaufnehmer`**, **`nenngeschwindigkeit`**, **`material`**, **`tacho`**, **`elektronik`**, **`dms_nr`**, **`position`**, **`geliefert_ueber`**, **`projekt`**, **`bemerkungen`** (gleiche Semantik wie Dispo-Web `api/anlagenstamm_save.php`; **`aktueller_kunde`** / **`letzter_besuch`** werden nicht per API gesetzt). Erfolg: **`ok`**, **`id`**.
